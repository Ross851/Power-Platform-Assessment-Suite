...
import { UploadCloud, FileText, Trash2, Loader2, ExternalLink, MessageSquarePlus } from "lucide-react"
...
export function EvidenceManager({ projectName, questionId, standardSlug, initialEvidence }: EvidenceManagerProps) {
  ...
  const fetchEvidence = useCallback(async () => {
    setIsLoading(true)
    const fetchedEvidence = await getEvidenceForQuestion(projectName, questionId)
  ...
  const onDrop = useCallback(
    ...
    [projectName, questionId, evidence, startTransition],
  )
  ...
  const handleAddSnippet = () => {
    ...
    startTransition(async () => {
      ...
      const result = await addQuestionEvidence(formData)
      if (result.error) {
        alert(`Failed to add snippet: ${result.error}`)
      } else if (result.newEvidence) {
        const updatedEvidence = [...evidence, result.newEvidence]
        setEvidence(updatedEvidence)
        updateStore(updatedEvidence)
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <Textarea
            ...
            disabled={isPending}
          />
          <Button onClick={handleAddSnippet} disabled={isPending || !snippet.trim()} size="sm">
